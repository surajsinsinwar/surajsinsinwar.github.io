...
function draw() {
  ...
  print_stats(
    mid,
    120,
    "Total individuals reached to the target in current generation",
    completed
  );
  print_stats(
    mid,
    140,
    "Maximum individuals reached to the target in single generation",
    max_till_now
  );
  fill(255);
...
function population() {
  ...
  this.selection = function () {
    ...
    for (
      var i = 0;
      i < Math.min(top_50, population_slider_var.value - 1);
      i++
    ) {
    ...
    for (
      var i = Math.min(top_50, population_slider_var.value - 1);
      i < population_slider_var.value;
      i++
    ) {
